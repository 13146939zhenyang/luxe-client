	...
	return (
		<div className='w-full flex justify-center items-center bg-transparent flex-col'>
			...
			<div className='w-full flex lg:justify-end justify-evenly items-center lg:text-base text-sm font-base lg:gap-10'>
				{navbar.map(
					(item, index) => (
						<motion.a href={item.position} variants={fadeIn("right", "spring", index * 0.5, 0.75)} className='text-gray-300 hover:text-white transition duration-200 ease-in-out drop-shadow-2xl'>{item.name.toUpperCase()}</motion.a>
					)
				)}
			</div>
		</div>
